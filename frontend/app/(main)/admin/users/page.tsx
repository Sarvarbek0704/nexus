...
const ROLE_COLORS: Record<string, string> = {
  ...
  freelancer: "bg-emerald-100 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400",
  agency_owner: "bg-purple-100 text-purple-700 dark:bg-purple-950/30 dark:text-purple-400",
};
...
export default function AdminUsersPage() {
  ...
  const handleStatusChange = async (userId: string, newStatus: string) => {
    try {
      ...
      toast.success(t.adminExtended.users.toast.updated);
      refetch();
    } catch (err: any) {
    ...
    <div className="p-4 sm:p-6 space-y-5">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
          {t.adminExtended.users.title}
        </h2>
        <p className="text-gray-500 dark:text-gray-400 mt-1">
          {meta?.total
            ? `${meta.total.toLocaleString()} ${t.adminExtended.users.subtitle}`
            : t.adminExtended.users.subtitleAll}
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder={t.adminExtended.users.search}
        ...
        >
          <option value="">{t.adminExtended.users.allRoles}</option>
          <option value="client">{t.register.roles.client}</option>
          <option value="freelancer">{t.register.roles.freelancer}</option>
          <option value="agency_owner">{t.register.roles.agency}</option>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-16 text-gray-400">
            ...
            <p>{t.adminExtended.users.noUsers}</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-800">
                  {[
                    t.adminExtended.users.headers.user,
                    t.adminExtended.users.headers.role,
                    t.adminExtended.users.headers.status,
                    t.adminExtended.users.headers.joined,
                    t.adminExtended.users.headers.lastActive,
                    t.adminExtended.users.headers.actions,
                  ].map((h) => (
                    <th
                      key={h}
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1">
                        {user.status === "active" ? (
                          ...
                          >
                            {t.adminExtended.users.activate}
                          </button>
                        )}
                        {!user.isVerified && (
