  ...
  if (error || !project) {
    return (
      <div className="flex flex-col items-center justify-center min-h-96 gap-4">
        ...
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Project not found</h3>
        <Link href="/projects" className="text-nexus-600 hover:underline">Browse projects</Link>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
            ...
            <div className="flex items-start justify-between gap-4 mb-4">
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-gray-500 dark:text-gray-400">
                  <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> Posted {formatRelativeTime(project.createdAt)}</span>
                  <span className="flex items-center gap-1"><Eye className="w-3.5 h-3.5" /> {project.viewCount ?? 0} views</span>
                  <span className="flex items-center gap-1"><Users className="w-3.5 h-3.5" /> {project._count?.bids ?? 0} bids</span>
                </div>
              </div>
            </div>

            {/* Tabs */}
            <div className="flex gap-0 border-b border-gray-100 dark:border-gray-800 -mb-6 mt-4">
              {['overview', 'bids', 'attachments'].map((tab) => (
                <button
                  key={tab}
          ...
          {activeTab === 'overview' && (
            <div className="space-y-5">
              ...
              {project.milestones && project.milestones.length > 0 && (
                <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
                  <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Proposed Milestones</h2>
            ...
            <div className="space-y-4">
              {!isOwner && (
                <div className="p-4 bg-nexus-50 dark:bg-nexus-950/30 rounded-xl border border-nexus-100 dark:border-nexus-900 text-sm text-nexus-700 dark:text-nexus-300">
                  Only the project owner can see bid details.
                </div>
              )}
              {isOwner && project.bids && project.bids.length > 0 ? (
                ...
                <div className="text-center py-12 text-gray-400">No bids yet.</div>
              )}
            </div>
          )}
          ...
          {activeTab === 'attachments' && (
            <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
              <p className="text-gray-400 text-sm text-center py-8">No attachments for this project.</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
            <div className="mb-5">
              ...
              <p className="text-sm text-gray-400 capitalize mt-1">{project.type} price</p>
            </div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
            ...
            <div className="space-y-3">
              {[
                { label: 'Category', value: project.category?.name, icon: Tag },
                { label: 'Experience', value: project.experienceRequired, icon: Star, capitalize: true },
                { label: 'Deadline', value: project.deadline ? formatDate(project.deadline) : 'Flexible', icon: Calendar },
                { label: 'Posted', value: formatDate(project.createdAt), icon: Clock },
              ].map((item) => (
            ...
            <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400">
                  ...
                  {project.client.clientProfile?._count?.projects ?? 0} projects posted
                </div>
