...
import {
  DollarSign, ArrowDownLeft, ArrowUpRight, Lock, Plus, Minus,
...
const TYPE_TABS = ["all", "milestone_payment", "escrow_deposit", "escrow_release", "withdrawal", "refund"];
...
export default function PaymentsPage() {
  ...
  const handleDeposit = async () => {
    ...
    if (!amount || amount <= 0) {
      toast.error(t.paymentsPage.toast.invalidAmount);
      return;
    }
    try {
      await deposit({ amount, method: "wallet" }).unwrap();
      toast.success(`$${amount.toFixed(2)} ${t.paymentsPage.toast.depositSuccess}`);
      setDepositAmount("");
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      toast.success(`$${amount.toFixed(2)} ${t.paymentsPage.toast.withdrawSuccess}`);
      setWithdrawAmount("");
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{t.paymentsPage.title}</h2>
        <p className="text-gray-500 dark:text-gray-400 mt-1">{t.paymentsPage.subtitle}</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-nexus-600 to-nexus-700 rounded-xl p-6 text-white">
          <div className="flex items-start justify-between mb-4">
            <div>
              <p className="text-nexus-100 text-sm">{t.paymentsPage.availableBalance}</p>
          ...
          <div className="flex gap-2">
            ...
            >
              <Plus className="w-4 h-4" /> {t.paymentsPage.deposit}
            </button>
            <button
              onClick={() => { setShowWithdraw(true); setShowDeposit(false); }}
              className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-white/20 hover:bg-white/30 rounded-lg text-sm font-medium transition-colors"
            >
              <Minus className="w-4 h-4" /> {t.paymentsPage.withdraw}
            </button>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">{t.paymentsPage.inEscrow}</p>
              <p className="text-2xl font-bold text-orange-600 dark:text-orange-400 mt-1">
                {formatCurrency(Number(user?.escrowBalance ?? 0))}
              </p>
              <p className="text-xs text-gray-400 mt-1">{t.paymentsPage.escrowDesc}</p>
            </div>
          ...
          <div className="flex items-start justify-between">
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {user?.role === "client" ? t.paymentsPage.totalSpent : t.paymentsPage.totalEarned}
              </p>
              <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400 mt-1">
                {formatCurrency(user?.role === "client" ? (stats?.totalSpent ?? 0) : (stats?.totalEarned ?? 0))}
              </p>
              <p className="text-xs text-gray-400 mt-1 flex items-center gap-1">
                <TrendingUp className="w-3 h-3" /> {t.paymentsPage.allTimeTotal}
              </p>
            </div>
      ...
      {(showDeposit || showWithdraw) && (
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
          <h3 className="font-semibold text-gray-900 dark:text-white mb-4">
            {showDeposit ? t.paymentsPage.depositTitle : t.paymentsPage.withdrawTitle}
          </h3>
          <div className="flex items-center gap-3 max-w-md flex-wrap">
            ...
            >
              {(depositing || withdrawing) && <Loader2 className="w-4 h-4 animate-spin" />}
              {showDeposit ? t.paymentsPage.addFunds : t.paymentsPage.withdrawBtn}
            </button>
            <button
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800">
        <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-gray-800">
          <h3 className="font-semibold text-gray-900 dark:text-white">{t.paymentsPage.transactionHistory}</h3>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 text-gray-400">
            ...
            <p className="text-sm">{t.paymentsPage.noTransactions}</p>
          </div>
        ) : (
          <div className="divide-y divide-gray-100 dark:divide-gray-800">
            {payments.map((payment) => {
              const Icon = PAYMENT_TYPE_ICONS[payment.type] || DollarSign;
              const isCredit = ["milestone_payment", "escrow_release", "refund", "bonus", "deposit"].includes(payment.type);
              ...
              return (
                <div key={payment.id} className="flex items-center gap-4 px-5 py-4">
                  <div className={cn(
                    ...
                    <Icon className={cn("w-4 h-4", isCredit ? "text-emerald-600 dark:text-emerald-400" : "text-red-500")} />
                  </div>
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className={cn("font-semibold text-sm", isCredit ? "text-emerald-600 dark:text-emerald-400" : "text-red-500")}>
                      {isCredit ? "+" : "-"}{formatCurrency(payment.amount)}
                    </p>
                    <p className="text-xs text-gray-400 mt-0.5">{formatDate(payment.createdAt)}</p>
                  </div>

                  <span className={cn("px-2 py-0.5 rounded-full text-xs font-medium capitalize ml-2", getStatusColor(payment.status))}>
                    {payment.status}
                  </span>
                </div>
              );
